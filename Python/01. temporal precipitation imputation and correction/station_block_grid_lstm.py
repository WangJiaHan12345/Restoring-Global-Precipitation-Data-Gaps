...
###读取数据###
# 训练数据
# climates = ['shirun','banshirun','banganhan','ganhan']
# for climate in climates:
#     earths=['北']
#     for earth in earths:
#         seasons=['9-11','12-2']
#         for season in seasons:
#             blocks = ['B','C','D']   #视具体情况改变块数
#             for block in blocks:
climate = 'ganhan'
earth = '北'
season = '12-2'
...
for block in blocks:
    ...
    train_wendu_path = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\xunlian_features4\\wendu\\' + block + '\\'
    ...
    # 还没保存结果
    path_random_save = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\result(有辅助因子二层)\\' + earth + '\\' + season + '\\'
    ...
    test_wendu_path = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\ceshi_features4\\wendu\\' + block + '\\'
    ...
    # time_step= 23 # 3-5 6-8:23  9-11: 30  12-2:29
        ...
        with tf.Session() as sess:
            ...
            for i in range(25):
                for step in range(len(batch_index)-1):
                    # # 自注意力机制
                    # X_1 = train_x[batch_index[step]:batch_index[step+1]]
                    # Y_1 = train_y[batch_index[step]:batch_index[step+1]]
                    #
                    # with tf.Session() as sess:
                    #     X_1 = np.array(X_1)
                    #     X_1 = SE_Block(X_1)
                    ...
                    #     Y_1 = sess.run(Y_1)
                    #
                    # _,loss_=sess.run([train_op,loss],feed_dict={X:X_1,Y:Y_1,keep_prob:0.8})
