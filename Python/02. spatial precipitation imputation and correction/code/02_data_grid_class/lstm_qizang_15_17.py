...
for season in seasons:
    blocks = ['A','B','C']  #,'B','C'
    for block in blocks:
        a_path = 'H:\\青藏高原数据\\时间预测\\2015-2017\\03_class_data\\'+ season +'\\xunlian\\gsmap_gauge\\'+ block +'\\'
        # train_dem_path = 'H:\\青藏高原数据\\时间预测\\2015-2016(final)\\03_class_data\\' + season + '\\xunlian\\dem\\' + block + '\\'
        # train_lat_path = 'H:\\青藏高原数据\\时间预测\\2015-2016(final)\\03_class_data\\'+ season +'\\xunlian\\lat_wei\\'+ block +'\\'
        # train_wendu_path = 'H:\\青藏高原数据\\时间预测\\2015-2016(final)\\03_class_data\\'+ season +'\\xunlian\\wendu\\'+ block +'\\'
        c_path = 'H:\\青藏高原数据\\时间预测\\2015-2017\\03_class_data\\'+ season +'\\xunlian\\国家气象局\\'+ block +'\\'
        d_path = 'H:\\青藏高原数据\\时间预测\\2015-2017\\03_class_data\\'+ season +'\\xunlian\\gsmap_mvk\\'+ block +'\\'
        ...
        # 还没保存结果
        path_random_save = 'H:\\青藏高原数据\\时间预测\\2015-2017\\result(分块_无辅助因子)\\' + season +'\\'
        ...
        # 测试数据
        a_path_1 = 'H:\\青藏高原数据\\时间预测\\2015-2017\\03_class_data\\' + season + '\\ceshi\\gsmap_gauge\\' + block + '\\'
        # test_dem_path = 'H:\\青藏高原数据\\时间预测\\2015-2016(final)\\03_class_data\\' + season + '\\ceshi\\dem\\' + block + '\\'
        # test_lat_path = 'H:\\青藏高原数据\\时间预测\\2015-2016(final)\\03_class_data\\' + season + '\\ceshi\\lat_wei\\' + block + '\\'
        # test_wendu_path = 'H:\\青藏高原数据\\时间预测\\2015-2016(final)\\03_class_data\\' + season + '\\ceshi\\wendu\\' + block + '\\'
        # c_path_1 = 'H:\\青藏高原数据\\时间预测\\2015-2017\\03_class_data\\' + season + '\\ceshi\\国家气象局\\' + block + '\\'
        # d_path_1 = 'H:\\青藏高原数据\\时间预测\\2015-2017\\03_class_data\\' + season + '\\ceshi\\gsmap_mvk\\' + block + '\\'
        ...
        test_data = np.zeros((test_count, 2))
        ...
        # 用来存放a_in_1 和 d_in_1
        test_data[:, [1]] = a_in_1[:, [0]]
        # test_data[:, [3]] = d_in_1[:, [0]]


        ##############################################################################################
        ###定义设置LSTM常量###
        rnn_unit = 30 # 大数据用60  小数据用30
        # 隐层单元的数量  100适用于小的
        input_size = 1 # 输入矩阵维度
        ...
        def get_train_data_12_2(batch_size=batch_size, time_step=time_step): #90是按照格网数来的  或者将5改为73
            ...
            for num in range(len(normalized_train_data) // 181):  #271代表总的天数
                for i in range(59 - time_step):
                    w = i + num * 181
                    if i % batch_size == 0:
                        batch_index.append(w)
                batch_index.append((181 * num + 59 - time_step))

                for i in range(59,150 - time_step):
                    w = i + num * 181
                    if (i-59) % batch_size == 0:
                        batch_index.append(w)
                batch_index.append((181 * num + 150 - time_step))


                for i in range(150,181 - time_step):
                    w = i + num * 181
                    if (i-150) % batch_size == 0:
                        batch_index.append(w)
                batch_index.append((181 * (num+1) - time_step))
        ...
        def get_test_data_12_2(time_step=time_step):
            ...
            for num in range(len(normalized_test_data) // 90):  # 90代表12-2的天数
                for i in range(size1):  # 73
                    ...
                    m = test_data[w + i * time_step:w + (i + 1) * time_step, 1:]
        ...
        def get_train_data_3_8(batch_size=batch_size, time_step=time_step): #90是按照格网数来的  或者将5改为73
            ...
            for num in range(len(normalized_train_data) // 184):  #271代表总的天数
                for i in range(92 - time_step):
                    w = i + num * 184
                    if i % batch_size == 0:
                        batch_index.append(w)
                batch_index.append((184 * num + 92 - time_step))

                for i in range(92,184 - time_step):
                    w = i + num * 184
                    if (i-92) % batch_size == 0:
                        batch_index.append(w)
                batch_index.append((184 * (num + 1) - time_step))
        ...
        def get_train_data_9_11(batch_size=batch_size, time_step=time_step): #90是按照格网数来的  或者将5改为73
            ...
            for num in range(len(normalized_train_data) // 182):  #271代表总的天数
                for i in range(91 - time_step):
                    w = i + num * 182
                    if i % batch_size == 0:
                        batch_index.append(w)
                batch_index.append((182 * num + 91 - time_step))

                for i in range(91,182 - time_step):
                    w = i + num * 182
                    if (i-91) % batch_size == 0:
                        batch_index.append(w)
                batch_index.append((182 * (num+1) - time_step))
        ...
        ###############将预测的结果存放起来##################################
        path_predict_save = path_random_save + 'ANN\\'   # gauge_predict
        mkdir(path_predict_save)

        path_early_save = path_random_save + 'mvk\\'
        mkdir(path_early_save)


        path_final_save = path_random_save + 'gauge\\'    #gauge
