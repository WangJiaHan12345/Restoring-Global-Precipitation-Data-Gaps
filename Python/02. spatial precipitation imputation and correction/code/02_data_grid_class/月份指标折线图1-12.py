...
for season in seasons:
    if __name__ == '__main__':
        ...
        a_path = 'G:\\全球\\时间预测结果\\地面站点_final\\global_climate\\'+ season +'\\Early\\'
        b_path = 'G:\\全球\\时间预测结果\\地面站点_final\\global_climate\\'+ season +'\\ANN\\'
        c_path = 'G:\\全球\\时间预测结果\\地面站点_final\\global_climate\\'+ season +'\\cpc\\'
        d_path = 'G:\\全球\\时间预测结果\\地面站点_final\\global_climate\\'+ season +'\\Final\\'
        e_path = 'G:\\全球\\时间预测结果\\地面站点_final\\global_climate\\'+ season +'\\ANN_差\\'
        f_path = 'G:\\全球\\时间预测结果\\地面站点_final\\global_climate\\'+ season +'\\ANN_最差\\'
        ...
        result_n_cc = np.zeros((12, 1))
        result_n_bias = np.zeros((12, 1))
        result_n_rmse = np.zeros((12, 1))
        result_n_mae = np.zeros((12, 1))
        result_n_abias = np.zeros((12, 1))
        result_n_pod = np.zeros((12, 1))
        result_n_far = np.zeros((12, 1))
        result_n_csi = np.zeros((12, 1))
        ...
        step = [0,31,55,85,115,145,175,205,235,265,295,325,355]
        ...
        for i in range(1, 13):
            ...
            CC_n = cc_count(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]],  number1, number2)
            BIAS_n = bias_count(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]],  number1, number2)
            RMSE_n = rmse_count(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]], number1, number2)
            MAE_n = mae_count(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]],  number1, number2)
            ABIAS_n = abias_count(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]],  number1, number2)
            POD_n = pod(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]],  number1, number2)
            FAR_n = far(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]],  number1, number2)
            CSI_n = csi(f_in[:,step[i-1]:step[i]], c_in[:,step[i-1]:step[i]],  number1, number2)


            result_n_cc[[i-1], [0]] = CC_n
            result_n_bias[[i-1], [0]] = BIAS_n
            result_n_rmse[[i-1], [0]] = RMSE_n
            result_n_mae[[i-1], [0]] = MAE_n
            result_n_abias[[i-1], [0]] = ABIAS_n
            result_n_pod[[i-1], [0]] = POD_n
            result_n_far[[i-1], [0]] = FAR_n
            result_n_csi[[i-1], [0]] = CSI_n


        path_save = 'G:\\毕业论文图\\全球\\时间\\含有地面站点网格的图\\月份性能折线图\\' + season + '\\'
        ...
        e = path_save + 'result_n_cc.txt'
        with open(e, 'w')as f:
            for i in range(len(result_n_cc)):
                for j in range(1):
                    f.writelines(str(result_n_cc[i, j]))
                    f.writelines('\n')
        e1 = path_save + 'result_n_bias.txt'
        with open(e1, 'w')as f:
            for i in range(len(result_n_bias)):
                for j in range(1):
                    f.writelines(str(result_n_bias[i, j]))
                    f.writelines('\n')
        e2 = path_save + 'result_n_rmse.txt'
        with open(e2, 'w')as f:
            for i in range(len(result_n_rmse)):
                for j in range(1):
                    f.writelines(str(result_n_rmse[i, j]))
                    f.writelines('\n')
        e3 = path_save + 'result_n_mae.txt'
        with open(e3, 'w')as f:
            for i in range(len(result_n_mae)):
                for j in range(1):
                    f.writelines(str(result_n_mae[i, j]))
                    f.writelines('\n')

        e4 = path_save + 'result_n_abias.txt'
        with open(e4, 'w')as f:
            for i in range(len(result_n_abias)):
                for j in range(1):
                    f.writelines(str(result_n_abias[i, j]))
                    f.writelines('\n')
        e5 = path_save + 'result_n_pod.txt'
        with open(e5, 'w')as f:
            for i in range(len(result_n_pod)):
                for j in range(1):
                    f.writelines(str(result_n_pod[i, j]))
                    f.writelines('\n')
        e6 = path_save + 'result_n_far.txt'
        with open(e6, 'w')as f:
            for i in range(len(result_n_far)):
                for j in range(1):
                    f.writelines(str(result_n_far[i, j]))
                    f.writelines('\n')
        e7 = path_save + 'result_n_csi.txt'
        with open(e7, 'w')as f:
            for i in range(len(result_n_csi)):
                for j in range(1):
                    f.writelines(str(result_n_csi[i, j]))
                    f.writelines('\n')
