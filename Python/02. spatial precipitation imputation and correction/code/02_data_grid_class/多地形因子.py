...
earth = '北'
season = '6-8'
blocks = ['A','B','C','D']  #'A','B','C','D','E',
for block in blocks:
    #要不要加上坡度和坡向还有高程
    a_path = 'H:\\时间预测\\四个区域可用数据\\'+ climate +'\\只选用有地面站点\\03_final_ns_data\\'+earth+'\\'+ season +'\\xunlian4\\Final\\' + block + '\\'
    train_dem_path = 'H:\\时间预测\\四个区域可用数据\\'+ climate +'\\只选用有地面站点\\03_final_ns_data\\'+earth+'\\'+ season +'\\xunlian_features4\\dem\\' + block + '\\'
    train_lat_path = 'H:\\时间预测\\四个区域可用数据\\'+ climate +'\\只选用有地面站点\\03_final_ns_data\\'+earth+'\\'+ season +'\\xunlian_features4\\lat_wei\\' + block + '\\'
    # train_slope_path = 'H:\\时间预测\\四个区域可用数据\\'+ climate +'\\只选用有地面站点\\03_final_ns_data\\'+earth+'\\'+ season +'\\xunlian_features4\\Slope\\' + block + '\\'
    c_path = 'H:\\时间预测\\四个区域可用数据\\'+ climate +'\\只选用有地面站点\\03_final_ns_data\\'+earth+'\\'+ season +'\\xunlian4\\cpc\\' + block + '\\'
    d_path = 'H:\\时间预测\\四个区域可用数据\\'+ climate +'\\只选用有地面站点\\03_final_ns_data\\'+earth+'\\'+ season +'\\xunlian4\\Early\\' + block + '\\'
    ...
    path_random_save = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\result(分块_残差)\\' + earth + '\\' + season + '\\'
    ...
    # train_data[:, [3]] = train_slope[:, [0]]
    train_data[:, [3]] = c_in[:, [0]]


    # 测试数据
    a_path_1 = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\ceshi4\\Final\\' + block + '\\'
    test_dem_path = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\ceshi_features4\\dem\\' + block + '\\'
    test_lat_path = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\ceshi_features4\\lat_wei\\' + block + '\\'
    # test_slope_path = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\ceshi_features4\\Slope\\' + block + '\\'
    c_path_1 = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\ceshi4\\cpc\\' + block + '\\'
    d_path_1 = 'H:\\时间预测\\四个区域可用数据\\' + climate + '\\只选用有地面站点\\03_final_ns_data\\' + earth + '\\' + season + '\\ceshi4\\Early\\' + block + '\\'
    ...
    def lstm(X):
        ...
        output_rnn = output_rnn + input_rnn
    ...
    def test_lstm(time_step=time_step):
        ...
        with tf.Session() as sess:
            ...
            with open(path_predict_save +'_data.txt', 'w')as f:
            ...
            with open(path_early_save +'_data.txt', 'w')as f:
                for j in range(len(early_data)):
                    f.writelines(str(early_data[j][0]))
                    f.writelines('\n')


            with open(path_final_save +'_data.txt', 'w')as f:
                for j in range(len(final_data)):
                    f.writelines(str(final_data[j][0]))
                    f.writelines('\n')

            with open(path_cpc_save +'_data.txt', 'w')as f:
