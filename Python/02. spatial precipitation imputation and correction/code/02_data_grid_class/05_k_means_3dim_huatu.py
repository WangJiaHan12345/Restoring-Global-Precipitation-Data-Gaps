...
# 三种数据进行分块
path_rain_csv = "H:\\时间预测\\四个区域可用数据\\shirun\\只选用有地面站点\\rain_sum_n_3-5.csv"
...
# print(data_dem)
#
path_mrdem_csv = "H:\\时间预测\\四个区域可用数据\\shirun\\只选用有地面站点\\dem\\MRDEM.csv"
...
t = open('H:\\时间预测\\四个区域可用数据\\shirun\\只选用有地面站点\\标准化聚类结果\\class_sumrain_n_3-5.txt', 'w') #得到每个点是属于第几个聚类
...
plt.savefig("H:\\时间预测\\四个区域可用数据\\shirun\\只选用有地面站点\\标准化聚类结果\\n_3-5.png", bbox_inches='tight')
plt.show()
